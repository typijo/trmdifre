...
import trmdifre as transformer # *patched*

# from official.transformer.utils import dataset
import dataset_difre as dataset # *patched*
...
TRMTYPES = {
  "difre": transformer.TransformerDifre
}
...
def make_model_fn_difre(modeltype="difre"):
  tf.logging.info("Use a Transformer-difre model.")
  ...
  def model_fn(features, labels, mode, params):
    ...
    with tf.variable_scope("model"):
      ...
      if modeltype in TRMTYPES:
        model = TRMTYPES[modeltype](params, mode == tf.estimator.ModeKeys.TRAIN)
      else:
...
def define_transformer_flags():
  ...
  flags.DEFINE_enum(
      name="trm_type", default="difre",
      enum_values=TRMTYPES.keys(),
      help=flags_core.help_wrap(
          "Transformer-difre Model used. "
          "'difre' to use the first Transfomer-difre model"))
...
def construct_estimator(flags_obj, params, schedule_manager):
  ...
  ### draw an adequate model_fn
  model_fn_to_use = make_model_fn_difre(flags_obj.trm_type)
  ...
  if not params["use_tpu"]:
    ...
    return tf.estimator.Estimator(
        model_fn=model_fn_to_use, model_dir=flags_obj.model_dir, params=params,
