...
1. Batching scheme
   ...
   Trmdifre examples encoded as follows:
    {
      "old_jp": [variable length array of integers],
      "old_en": [variable length array of integers],
      "new_jp": [variable length array of integers],
      "new_en": [variable length array of integers]
    }

   While the original Transformer data encoded as follows:
   --------------------
   ...
   Other process is same as the original one, that is:
   -------------------
   ...
   -------------------
   except that batched dataset is different:

     {
       "inputs": [group_batch_size, 3, padded_input_length],
       "targets": [group_batch_size, padded_target_length]
     }
   
   where inputs[i] has [i-th oldsrc, i-th oldtrg, i-th newsrc]
...
def _parse_example(serialized_example):
  ...
  data_fields = {
      "old_jp": tf.VarLenFeature(tf.int64),
      "old_en": tf.VarLenFeature(tf.int64),
      "new_jp": tf.VarLenFeature(tf.int64),
      "new_en": tf.VarLenFeature(tf.int64)
  }
  parsed = tf.parse_single_example(serialized_example, data_fields)
  
  inputs_oj = tf.sparse_tensor_to_dense(parsed["old_jp"])
...
def _stack_example(oj, oe, nj, ne):
  ...
  inputs = tf.stack([oj, oe, nj], axis=1)
...
def _get_example_length(example):
  ...
  **Not used.**
  """
  length = tf.maximum(
    tf.maximum(tf.shape(example[0])[1], tf.shape(example[1])[0]),
    tf.maximum(tf.shape(example[1])[1], tf.shape(example[2])[0])
  )
...
def _batch_examples(dataset, batch_size, max_length):
  ...
  **Not used.**
  ...
  def example_to_bucket_id(
    example_input_oldsrc, example_input_oldtrg, 
    example_input_newsrc, example_target):
    """Return int64 bucket id for this example, calculated based on length."""
    seq_length = _get_example_length(
      (example_input_oldsrc, example_input_oldtrg,
       example_input_newsrc, example_target))
...
def _read_and_batch_from_files(
  ...
  Args:
    ...
    repeat: Number of times to repeat the dataset. If None, the dataset is
      repeated forever.
    static_batch: **Not worked. Always static_batch=True**
      Whether the batches in the dataset should have static shapes.
  ...
  ## *Not used* because dataset already padded
  # dataset = dataset.filter(lambda xs, y: _filter_max_length((xs, y), max_length))
...
def train_input_fn(params, trm_type="trm"):
...
def eval_input_fn(params, trm_type="trm"):
